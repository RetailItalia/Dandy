    ...
    public abstract partial class TestSuite
    {
        ...
        public void NullDateTime()
        {
            using (var connection = GetOpenConnection())
            {
                ...
                var stuff = connection.GetAll<Stuff>().ToList();
        ...
        public void InsertGetTimeField()
        {
            using (var connection = GetOpenConnection())
            {
                ...
                var id = connection.Insert(new TimeField { TimeValue = expected });
                var actual = connection.Get<TimeField>(id).TimeValue.Value;
        ...
        public async void InsertGetUpdateAliasedFieldAsync()
        {
            using (var connection = GetOpenConnection())
            {
                SqlMapperExtensions
                    .RegisterMap<AliasedField>(new AliasedField.AliasMapper());
                connection.DeleteAll<AliasedField>();
                var id = await connection.InsertAsync(new AliasedField() { AField = "Adam" });
